...
public class SpaceMenuFreePlay : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (SteamVR_Actions.default_menu.GetState(handSource) == true)
        {
            ...
            {
                currentMenu = Instantiate(menu, new Vector3(player.transform.position.x + 4, player.transform.position.y + 1, player.transform.position.z), Quaternion.Euler(0, 90, 0));
